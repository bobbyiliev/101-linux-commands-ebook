...
def test_hello_command_with_verbose():
    """Test the hello command with --verbose flag."""
    result = run_cli(["--verbose", "hello", "greet"])
    assert result.returncode == 0
    assert "Hello, World!" in result.stdout
...
def test_list_command_with_verbose():
    """Test the list command with --verbose flag."""
    result = run_cli(["--verbose", "list"])
    assert result.returncode == 0
    assert "ls - List directory contents." in result.stdout
    assert "cd - Change directory." in result.stdout
    assert "pwd - Print working directory." in result.stdout
...
def test_version_command_with_verbose():
    """Test the version command with --verbose flag"""
    result = run_cli(["--verbose", "version"])
    assert result.returncode == 0
    assert "101-linux v" in result.stdout
    assert "0.1.0" in result.stdout
    assert "verbose" in result.stdout.lower()
...
def test_show_ls_verbose():
    """Test the show command with ls and --verbose flag."""
    result = run_cli(["--verbose", "show", "ls"])
    assert result.returncode == 0
    assert "ls" in result.stdout
    assert "List" in result.stdout
    assert "verbose" in result.stdout.lower()
...
def test_show_grep_verbose():
    """Test the show command with grep and --verbose flag."""
    result = run_cli(["--verbose", "show", "grep"])
    assert result.returncode == 0
    assert "grep" in result.stdout
    assert "Search" in result.stdout or "Print" in result.stdout
    assert "verbose" in result.stdout.lower()
...
if __name__ == "__main__":
    ...
    test_hello_command_with_verbose()
    test_hello_help()
    test_list_command()
    test_list_command_with_verbose()
    test_version_command()
    test_version_show_command()
    test_version_command_with_verbose()
    test_show_ls()
    test_show_ls_verbose()
    test_show_grep()
    test_show_grep_verbose()
    test_show_invalid()
    test_search_match()
